...
* `bad_data/orders/<client_code>/<period>` - данные, содержащие синтаксические ошибки, для последующего анализа
* `stats/orders/<client_code>/<period>` - статистика по корректным данным, для мониторинга семантического качества данных
...
1. Quality Gate - фильтрация синтаксически неверных записей. С каждым клиентом
   отдельно обговариваются условия, при котором запись будет помещена в область
   Bad Data.

2. Загрузка синтаксически корректных данных в стейджинговый массив данных Staging, в котором данные представлены в очищенном виде и приведены к единому виду для всех источников:
...
3. Statistics - После загрузки стейджинга рассчитываются статистические метрики по данным,
   для выявления неоднородности и некачественных данных. Метрики сохраняются на
   диск в директорию `stats/orders/<client_code>/<period>` для последующего анализа.

4. Загрузка Покупателей из массива Staging:
...
5. Из массива Staging формируется массив Orders, содержащий информацию о
   покупках за текущий период.

6. Массив Customers (2) соединяется левым соединением (LEFT JOIN) с массивом
...
**Важно!** В `run_etl.sh` прописан путь к директории, в которой установлен Spark, перед запуском замените на актуальный путь:
...
Для запуска тестов использовать команду `mvn test`. Используется ScalaTest.


## Проблемы и открытые вопросы

1. [ ] Как корректно обрабатывать неправильные данные? Есть вариант использовать typeclass Option в Scala со значением по-умолчанию для невалидных данных. Другой вариант - использовать схему данных в SparkSQL, тогда при преобразовании в DataFrame невалидные значения будут заменены на null. В текущей версии используется комбинированный подход - при некорректном или пустом значении атрибута PaymentSum в результирующем наборе данных значение будет заменено на 0. При некорректном значении для даты при вычислении "Стажа пользователя" результирующим значением будет null
2. [x] Не до конца понятно, для каких задач лучше использовать RDD а для каких - DataFrame (DataSets)
3. [x] Как правильно писать тесты
